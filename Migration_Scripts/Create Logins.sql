SET NOCOUNT ON
-- Scripting Out the Logins To Be dropped because of the different SID
SELECT 'IF (SUSER_ID('+QUOTENAME(SP.name,'''')+') IS NOT NULL) AND EXISTS(SELECT name FROM sys.syslogins WHERE name ='''+SP.name+''' and sid != '+CONVERT(NVARCHAR(MAX),sl.sid,1)+') 
		DROP LOGIN ' + QUOTENAME(SP.name) + ';' AS [-- SQL Logins To Be Dropped --]
FROM sys.server_principals AS SP JOIN sys.sql_logins AS SL
...
SELECT 'IF (SUSER_ID('+QUOTENAME(SP.name,'''')+') IS NULL) 
		CREATE LOGIN ' +QUOTENAME(SP.name)+
			   CASE 
					...
					THEN ' WITH PASSWORD = ' +CONVERT(NVARCHAR(MAX),SL.password_hash,1)+ ' HASHED, ' + 
						 ' SID = '+ CONVERT(NVARCHAR(MAX),sl.sid,1) +
				...
				END 
	   +' DEFAULT_DATABASE=[' +SP.default_database_name+ '], DEFAULT_LANGUAGE=[' +SP.default_language_name+ '];' COLLATE SQL_Latin1_General_CP1_CI_AS AS [-- Logins To Be Created --]
